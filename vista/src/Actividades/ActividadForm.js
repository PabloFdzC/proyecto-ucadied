...
//Solo está creado hay que cambiar lo de adentro
class ActividadForm extends React.Component {
    ...
    async crearActividad(evento){
        ...
        try{
            const resp = await this.queriesGenerales.postear("/juntaDirectiva/crearActividad", datos);
            this.setState({
                creado:true,
            });
    ...
    agregarActividad(){
        ...
        if(!this.state.errores.hayError){
            this.props.agregaActividad(this.state.campos);
        }
    }
        ...
        {this.props.soloCampos ? 
            <>
                ...
                <div className="d-flex justify-content-end">
                    <div className="m-1">
                        <button type="button" onClick={this.agregarActividad} className="btn btn-primary">Agregar puesto</button>
                    </div>
                </div>
            </> : 
            <>
                ...
                {!this.state.creado ? 
                <form onSubmit={this.crearActividad} className="needs-validation" noValidate>
                    {campos}
                    <div className="d-flex justify-content-end">
                        <div className="m-1">
